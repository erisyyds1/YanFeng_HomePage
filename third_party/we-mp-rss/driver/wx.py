...
class Wx:
    ...
    async def wxLogin(self, CallBack=None, NeedExit=True):
        ...
        try:
            ...
            # 定位二维码区域。微信登录页结构会调整，优先等待真实登录二维码图片。
            try:
                qrcode = await page.wait_for_selector(
                    "img[src*='scanloginqrcode'], img[src*='loginqrcode']",
                    timeout=15 * 1000,
                )
            except Exception:
                print_warning("未等到真实二维码图片，尝试二维码区域截图兜底")
                qrcode = await page.wait_for_selector(
                    ".login__type__container__scan__qrcode img, .login__type__container__scan__qrcode",
                    timeout=5 * 1000,
                )
            ...
            saved_qrcode = False
            if code_src and ("scanloginqrcode" in code_src or "loginqrcode" in code_src):
                qrcode_url = urljoin(self.WX_LOGIN, code_src)
                response = await page.context.request.get(qrcode_url, headers={"Referer": self.WX_LOGIN})
                if response.ok:
                    with open(self.wx_login_url, "wb") as file:
                        file.write(await response.body())
                    saved_qrcode = True

            # 使用 Playwright 截图功能兜底，避免只保存微信页面占位图。
            if not saved_qrcode:
                await wait_for(qrcode.screenshot(path=self.wx_login_url), timeout=10)
