...
const ACTIVITY_GLOW_COLORS = ['#c8322a', '#f4d06f', '#ffffff'];
const ACTIVITY_GLOW_PROPS = {
  edgeSensitivity: 18,
  glowColor: '4 78 62',
  borderRadius: 0,
  glowRadius: 30,
  glowIntensity: 0.82,
  coneSpread: 28,
  colors: ACTIVITY_GLOW_COLORS,
  fillOpacity: 0.26
};
const ACTIVITY_TAG_GLOW_PROPS = {
  edgeSensitivity: 12,
  glowColor: '4 78 62',
  borderRadius: 0,
  glowRadius: 14,
    ...
    <>
      ...
      <div className="relative mx-auto grid h-full max-w-[1600px] grid-rows-[1fr_1fr_1fr_auto] gap-3">
        <div className="grid min-h-0 grid-cols-[0.48fr_1.52fr] gap-3 lg:grid-cols-[0.42fr_1.58fr]">
          <BorderGlow {...ACTIVITY_GLOW_PROPS} backgroundColor="#111111" className="min-h-0">
            <article className="relative h-full overflow-hidden p-5 text-white md:p-6">
          ...
          <BorderGlow {...ACTIVITY_GLOW_PROPS} backgroundColor="#050505" className="min-h-0">
            <figure className="relative h-full min-h-0 overflow-hidden bg-black">
        ...
        <BorderGlow {...ACTIVITY_GLOW_PROPS} backgroundColor="#080808" glowRadius={24} fillOpacity={0.14}>
          <div className="relative overflow-hidden px-4 py-3">
            <div className="flex flex-wrap items-center gap-2 md:gap-3">
              ...
              {['放映会', '组活', '轻音 Live', '合宿', '联合观影', '更多日常活动'].map((event) => (
                <BorderGlow key={event} {...ACTIVITY_TAG_GLOW_PROPS} backgroundColor="#111111" className="border-glow-chip shrink-0">
                  <span className="px-3 py-1.5 text-xs font-black text-white/72">{event}</span>
                </BorderGlow>
              ))}
            </div>
          </div>
